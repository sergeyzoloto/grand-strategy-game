...
implemented `NAT-080` (Optical Theory: Refraction, Lenses, and Magnified
...
`lens|optic` anywhere in `domains/engineering/`. NAT-080 covers the
...
Optical Instrument Manufacture," prereqing `NAT-080`, enabling nothing
further required but see-alsoing `NAT-120` (Heliocentrism) and `NAT-130`
(Germ Theory), both of which depend on the artifact NAT-080's own theory
alone doesn't build.
...
COM-020's successor cites as a contributing cause has no WAR-side account
...
audit built `LAW-070` (Civil Law: Property, Contract, and Inheritance)
...
entry that touches `contract enforcement` in its own stats (`COM-110`,
`COM-030`, `COM-090`, `FIN-040`, `FIN-060`) — not one of them links to
`LAW-070` in either `see_also` or `prereqs`. This isn't a homeless entry in
...
Fix: add `LAW-070` to `see_also` on all five entries, reciprocally.
...
That last sentence is aimed directly at `WAR-024`/`LAW-080` — see §4.
...
(REL-260/PHI-290 on State Shinto; the whole NAT-050/ENG chain) — and both
...
**Finding 1 — `WAR-024` (Feudal Land-Tenure Military Service) and `LAW-080`
...
into the samurai class's own hereditary bushido warrior code"_ — LAW-080's
...
direct `grep`. LAW-080 additionally covers the Roman colonate and the
Ethiopian gult system as region_variants WAR-024 doesn't touch, and WAR-024
covers the Islamic iqta system LAW-080 doesn't — so there is real,
...
(if any — check before retiring) get re-pointed to LAW-080.

**Finding 2 — `FIN-090` (General Insurance and Actuarial Risk-Pooling) and
...
insurance premium — as their central claim. FIN-090's own Timing section:
...
the archive. Checked directly: FIN-090's `see_also` is `[MAR-120]` only —
no HLT reference; HLT-033's `see_also` is `[HLT-070, HLT-210]` — no FIN
...
both halves: FIN-090's own _fire_-insurance content (the 1666 Great Fire,
property risk-pooling) is real, distinct, and unrelated to mortality
pricing specifically, and needs to survive. **Recommend:** retain FIN-090
...
material into FIN-090's existing life-insurance Regional-specifics content
as elaboration rather than duplication, and retire HLT-033 — but keep a
`see_also: [FIN-090]` pointer on `HLT-210` (Mutual Aid and Guild
...
**Considered and rejected as a duplicate:** `COM-050` (Caravan as
Risk-Pooling Institution) and `MIG-080` (Camel Caravan Mobility and Desert
Crossing) look like the same topic at a glance — both "caravan," both
desert trade — and are not: COM-050 covers merchants pooling capital and
guards as a _financing and risk-sharing_ institution (explicitly prereqing
a pack-animal mobility entry rather than re-deriving it), MIG-080 covers
the camel's own physiology as a _mobility-threshold_ unlock (explicitly
parallel to MIG-050's horse entry). This is the correct split — a
commercial institution built on top of a transport technology, the same
COM/MIG relationship that already works cleanly for COM-080/MIG-050 on the
...
1. **Feudalism and land-for-service obligation.** `LAW-080`/`WAR-024`
   (post-merge, one entry), `AGR-170` (Roman colonate, the direct
   institutional precursor LAW-080's own region_variant already names),
   `AGR-190` (serfdom), `AGR-200` (enclosure, the institution's eventual
...
2. **Colonization of the Americas.** `MIG-060`/`MIG-090`/`MIG-150`,
   `DIP-090`, `HLT-240`, `AGR-220`, touching `COM-140` (silver) and WAR's
   own conquest-doctrine content. Six domains, seven-plus entries. This one
   is self-nominated: MIG-150's own text already says, verbatim, _"a
   ...
   exist. The condition MIG-150 itself set has been met.

3. **Trade-network connectivity and disease transmission.** `HLT-170`
   (Connectivity Plagues), `HLT-220` (Black Death), `COM-080` (Silk Road,
   which HLT-170 already cites for the same route), `MIG-130` (Steppe
   Waves, the Mongol-peace trade integration HLT-170's own Black Death
   ...
   knowledge that HLT-170 tracks as pandemic-speed is COM-080's own
...
5. **Institutional confinement and reform.** `HLT-200` (The Asylum),
   `LAW-170` (Codified Criminal Law and Punishment Reform, the
   penitentiary/Beccaria entry), `REL-060` (Monasticism, the earlier
   custodial-institution precedent both later entries structurally
   echo), and `HLT-160` (Monastic Hospitals). Confirmed by direct check:
   `LAW-170` and `HLT-200` currently have zero cross-reference despite
...
- The five link-and-domains_touched fixes for LAW-070 in §1.
...
the-Americas thread page MIG-150 already asked for by name.** Those four
