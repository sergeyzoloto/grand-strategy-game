...
**379 → 377.** Two retirements (WAR-024 merged into LAW-005, HLT-033 merged
into FIN-022), no new entries, no other deletions. Confirmed by file count:
...
**1.2 — Merge WAR-024 into LAW-005.** Canonical LAW-005, `domains_touched`
...
no-smoothing rule. Updated `WAR-001` and `WAR-027`'s references from
WAR-024 to LAW-005. Tombstoned WAR-024 in `_meta/retired-ids.md`, file
...
LAW-005 already covers it): WAR-024's own south-asia `absence` note and its
...
**1.3 — Merge HLT-033 into FIN-022.** Canonical FIN-022, `health` added to
`domains_touched`. Folded HLT-033's England/Netherlands regional-specifics
sentence into FIN-022's Regional specifics verbatim. FIN-022's fire-insurance
content untouched. Per your decision (option b): HLT-032's directional
`enables: [HLT-033]` was dropped entirely rather than carried over as a
cross-domain prereq; HLT-032 instead got a plain `see_also: [FIN-022]`.
Redirected HLT-002 and HLT-029's `see_also` from HLT-033 to FIN-022; added
FIN-022's reciprocal `see_also` to HLT-002, HLT-029, and HLT-032. Tombstoned
...
confirmed thread member (MIG-004, MIG-005, MIG-012, DIP-007, HLT-017,
AGR-018, COM-011) for a WAR cross-reference — none exists in any direction.
...
**2.1 — LAW-017.** Added to `see_also` on COM-015, COM-012, COM-029,
FIN-021, FIN-005; those five added reciprocally to LAW-017. Commit
`5ba43a9`.

**2.2 — LAW-023 / HLT-030.** Cross-referenced in both directions. Commit
`e0b6904`.
...
entry's own frontmatter (e.g. "cohesion (WAR-007, already prereq) and
mobility range (MIG-001, MIG-003)" → MIG-001/003 get the same treatment as
WAR-007), and only where that produced no chronological inversion against
...
repeatedly — NAT-001/NAT-002, NAT-024/ENG-021, AGR-001/HLT-001,
WAR-002/MIG-002, COM-020/COM-024, ENG-014/COM-024, ENG-017/MIG-016,
REL-021's two citations), I resolved to `see_also` rather than force a
...
- **LAW-004 → LAW-005**: `see_also`. On the actual read, this is a
  confirmed comparative/alternative-model pairing, already reciprocated
  from LAW-005's own side ("a decentralized alternative to ... LAW-004")
  before this pass touched it.
- **WAR-005 ↔ WAR-007**: `see_also` both directions.
- **MAR-022's four citations** (MAR-005, MAR-008, MAR-012, MAR-024):
  `see_also` for all four.

Two citations that were genuine prereq/enables calls, not defaults:
**WAR-003 → WAR-001** and **MAR-010 → MAR-005** (both: "requires the
...
(HLT-032's directional link into HLT-033, and HLT-002's independent
