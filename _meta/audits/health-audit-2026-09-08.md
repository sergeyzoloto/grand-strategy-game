...
**The pattern, stated up front:** this is the most mechanically
sophisticated domain audited this cycle. It is the only domain that cites
...
What's missing is narrower than the mechanism list suggests: this domain
builds elaborate machinery for what happens once people are settled, fed,
diagnosed, and treated, but never models the deep-time baseline every one
of those machines silently measures itself against, or the far more common
household-level and communal channels through which most people in this
record actually got care.
...
institutional form (member-funded, not donor-funded) that this domain has
no entry for at all.
...
HLT section at all — COM/FIN, WAR/ENG, NAT/ENG, REL/PHI, and LAW/DIP each
get a dedicated pairing rule, and HLT is arguably the single
most-frequently-touched domain in the wiki (AGR, REL, ENG, PHI, LAW, COM,
WAR, NAT, and MIG entries all cross-reference it), yet the rule that would
...
entry in the domain I'd call a judgment call rather than an obvious fit —
not a violation, but the closest thing to one.

═══ PART B — PROPOSED ADDITIONS ═══

## LOAD-BEARING

**1. The Forager Baseline: Health and the Body Before Sedentism**
- Dates: -300000 to -10000, `era`. Regions: africa, middle-east, europe,
  east-asia.
- Answers: Probe 1 (Emergence Gap).
- Mechanism: A mobile forager's varied diet, dispersed low-density social
...
- Contested: true — how much of the "Neolithic decline" reading depends on
  ...
  favorable health profile as real but regionally variable (savanna
  foraging, coastal foraging, and circumpolar foraging carried different
  risk profiles), not a single uniform "natural" state.
- Confidence: medium.

**2. Household and Folk Medicine**
- Dates: -3000 to 1900, `era`. Regions: europe, middle-east, south-asia,
  east-asia, africa, americas.
- Answers: Probe 2 (Substrate Gap).
- Mechanism: A family member, neighbor, or local herbalist with orally-
...
- Hook: A modifier: population-level health outcomes track this
  substrate's coverage and quality far more than they track the codified
...
- Dates: 1250 to 1900, `century`. Regions: europe, middle-east, east-asia.
- Answers: Probe 3 (Scale Gap).
...
- Mechanism: A dedicated institution (Bethlem in London from 1247;
  ...
  county-asylum-building wave) confines and, depending on era and
  philosophy, either restrains or attempts to treat the mentally ill as a
  distinct medical population — converting madness from a diffuse social
  and family problem into an institutionally-managed, physically-separated
  category, with custodial control and genuine care pulling in the same
  direction only inconsistently.
- Hook: A modifier/branch: the asylum grants medical authority over
...
- Contested: true — how much genuine therapeutic improvement "moral
  ...
  inconvenient populations regardless of the era's stated philosophy, is
  a live historiographical dispute (associated with, among others,
  Foucault's own account); model custodial function and genuine
  therapeutic function as two separately-tracked variables rather than
  assuming either fully explains asylum-building in any given era.
- Confidence: high.

**5. Famine and Starvation as a Distinct Mortality Mechanic**
- Dates: -1800 to 1943, `era`. Regions: europe, south-asia, east-asia,
  africa.
- Answers: no single probe cleanly, but famine is one of this record's
  most consequential recurring mortality mechanics and has no entry
  anywhere in this domain, despite AGR-side crop-failure entries almost
  certainly assuming a health-side mortality consequence this domain never
  actually supplies.
- Mechanism: Sustained caloric deprivation directly causes death, but the
  larger share of famine mortality historically comes from starvation's
...
- Hook: An event: a food-supply collapse converts a population's ordinary,
  survivable background disease load into a mass-mortality event by
...
- Contested: true — the exact split between direct starvation mortality
  ...
  with the infectious-amplification share as the larger, more variable
  component, rather than treating famine as a single undifferentiated
  cause of death.
- Confidence: high.

## VALUABLE

**6. Public Health Statistics and Vital Registration**
...
- Mechanism: Systematic mortality recording (John Graunt's 1662 Bills of
  ...
  spatial disease mapping (John Snow's 1854 cholera map) convert disease
...
**7. Actuarial Life Insurance and the Pricing of Mortality Risk**
- Dates: 1693 to 1900, `century`. Regions: europe.
- Answers: Probe 6 (a distinct mechanism — individually-priced,
  commercial mortality-risk transfer, mechanically unlike Load-Bearing #3's
  community risk-pooling).
- Mechanism: Edmond Halley's 1693 mortality table converts age-specific
  death probability into a calculable, sellable price, letting a
  commercial insurer profitably underwrite individual mortality risk for
  a premium rather than requiring a bounded community of mutually-
  obligated members (Load-Bearing #3) — a market solution to the same
  underlying problem, priced rather than pooled by membership.
- Hook: A modifier: converts population-level mortality data (Valuable #6)
...
**8. Colonial and Tropical Medicine**
- Dates: 1750 to 1914, `century`. Regions: africa, south-asia,
  southeast-asia.
...
- Mechanism: European colonial administrations develop a dedicated
  ...
  institutional purpose is enabling and sustaining colonial personnel's
  presence in a disease environment they have no evolved or acquired
  resistance to, converting `medical efficacy` into a specific, targeted
  enabler of territorial control rather than a general population-health
  good.
- Hook: A gate: a targeted medical countermeasure (quinine prophylaxis
...
- Contested: true — how much colonial medical institutions genuinely
  ...
  local-population and colonizer health outcomes as two separately
  tracked effects of the same institution rather than assuming one
  implies the other.
- Confidence: high.
...
- Answers: nothing not already covered — this mostly restates HLT-004's
  ...
  mechanic.
- Hook: A modifier at best — a status- and sociability-linked hygiene
  venue, mostly flavor on top of mechanics this domain already has.
...
the entry's actual claim isn't about any one tradition's specific content;
it's the structural pattern (coherent theory drives durable legitimacy
independent of efficacy) recurring across all four independently, which
...
directly kill pathogens via a depleting resistance-pool mechanic
antisepsis doesn't have; psychoanalysis is a specific theoretical
framework's authority/efficacy split, the therapeutic turn is a broader
market-substitution-for-communal-function mechanic that happens to build
on psychoanalysis's cultural groundwork rather than restate it.

## RE-DOMAIN

No canonical-domain violations found (see Probe 7). The actionable finding
...
**Entry count:** 26 before. Proposed: +10 (5 load-bearing, 3 valuable, 2
optional), no cuts, no splits → **36**.

**Proposed out of symmetry rather than mechanical need:** both Optional
entries, named honestly in their own writeups — Spa/Bathing Culture and
Patent Medicine are each a flavor instance of a mechanic (commensality/
status-marking; authority decoupled from efficacy) this domain already
has fully built, included for good, vivid history rather than because the
domain's mechanics require them.

**Does this domain justify separate existence?** Yes, unambiguously — this
is the strongest case of any domain audited this cycle. Its
design-principle discipline (explicit citations, a genuinely load-bearing
authority/efficacy split sustained across six entries, two exemplary
depleting-pool mechanics) is unmatched elsewhere in the wiki, its boundary
practice is clean despite being the most cross-referenced domain audited,
and its prereq graph is nearly spotless apart from one clearly-evidenced
bug. The one real institutional gap — no HLT section in
`boundary-rules.md` despite the domain's heavy cross-referencing — is a
documentation debt the domain's own disciplined practice has been quietly
covering for, not a sign it doesn't deserve the section.
