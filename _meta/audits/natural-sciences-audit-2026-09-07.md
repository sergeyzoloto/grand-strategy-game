...
- **`NAT-010`** (prereqs `[]`) has no link to `AGR-050`, which already documents the practical token-and-tally accounting precursor to formal mathematical notation this entry's own Mechanism section assumes existed first.
...
- Prereqs: `NAT-010`. Enables: `NAT-180`, `NAT-210` (both currently missing this link).
- Contested: false.
...
Flavor entry on the single, extraordinary artifact (an ancient Greek geared analog astronomical computer, c. 150–100 BCE) recovered from a shipwreck in 1901. Real and genuinely startling history — nothing of comparable mechanical complexity is known to exist again for over a thousand years — but its broader significance (ancient astronomical and mechanical sophistication) is already covered by `NAT-010` and `NAT-040`, and as a single artifact it doesn't generalize into its own mechanism. Naming it because it's a great story, not because the domain needs it.
