...
as such rather than assumed: `AGR-007` (the ard/plow entry) contains no reference to
...
`HLT-026` (The Demographic Transitions) — cites the Hajnal line, the Western European
Marriage Pattern, late female marriage, permanent celibacy, neolocal residence, or
life-cycle service as a pre-modern fertility mechanism; `HLT-026` covers only the
...
| `REL-011` Parish Network Governance                                                   | household (registration)                              | Own Mechanism               | Yes, but about record-keeping                    | Parish-as-registry (baptism/marriage/burial records repurposed for state legibility). Not parish-as-governing-body (no vestry, churchwarden, or parish-meeting decision-making anywhere).                                                                                                                                                   |
| `HLT-027` The Forager Baseline                                                        | band (one clause)                                     | Own Mechanism (health)      | Yes, but not about group size                    | Diet/disease/activity baseline before sedentism. "Social units disperse and recombine" appears in one clause of the Social Consequence section; the entry never engages the coordination-cost or leadership-threshold question at all.                                                                                                      |
| `DIP-004` Gifts, Hostages, and Marriages                                              | marriage(s)                                           | Own Mechanism               | Yes, but elite-scoped                            | Royal/dynastic marriage as diplomatic collateral (heqin, Habsburg). Never reaches ordinary village- or lineage-level marriage-as-alliance.                                                                                                                                                                                                  |
| `AGR-014` The Pastoralist–Cultivator Conflict Cycle                                   | (land-use, not a term-list hit but directly relevant) | Own Mechanism               | Yes                                              | Land-use competition between two _already-formed_ social categories. Neither category's own internal composition is this entry's subject.                                                                                                                                                                                                   |
| `PHI-009`, `PHI-028`, `COM-027`, `FIN-020`, `WAR-023`, `LAW-012`, `HLT-010`           | kinship, prestige goods, chiefdom, baojia             | Purely incidental           | No                                               | Kinship as an ethical-philosophy comparison point (Mohism) or a structuralist-method example (Lévi-Strauss cited for _method_, not for alliance theory); "chiefdom" and "prestige goods" each used once as scale-descriptors; baojia covered twice, both times as a _policing/registration_ mechanism, never as village self-governance.    |
...
`Haudenosaunee`/`Iroquois` outside `LAW-003` and `DIP-003`, `Dunbar`, `fission-fusion`.
...
`PHI-036` comes closest — asabiyyah genuinely rises and mechanically declines — but its
...
`HLT-027` mentions social-unit dispersal in one clause; nothing anywhere cites Johnson's
...
The single most load-bearing gap found. `LAW-016` covers the _legal consequence_ of
kin-group liability without covering the kin-group's own boundary; `LAW-008`
_explicitly names and explicitly excludes_ lineage/clan corporate property-holding;
`REL-018` covers ancestor cult only as religious-authority origin. Unilineal vs.
...
`DIP-004` is real but elite-scoped (royal marriage as diplomatic collateral); nothing
...
milk kinship). The prompt's own suggestion that `AGR-007` already carries Goody/Alesina
content was checked and found false — see the note above.

### 4. Household Form — **ABSENT**

Zero hits for zadruga, joint family, stem family, dvor, or ie. `LAW-019` covers legal
_jurisdiction_ over marriage/inheritance, not household composition. `HLT-026` covers
...
`REL-012` covers canon law _courts_ as an institution and explicitly disclaims the
substantive law's content to `LAW-001`'s Manusmriti-adjacent territory — but neither
...
`REL-024` is genuine, well-built coverage of one branch: voluntary religious sodality
...
`WAR-023` ("a state or chiefdom"); "prestige goods" appears exactly once, describing
cacao-as-currency in `FIN-020`. Big man vs. ascribed chief, redistributive centers,
...
`PHI-036` genuinely covers asabiyyah — the prompt's own contingency ("if not, note
...
different mechanism from what `MIG-007` itself explicitly promises and never receives:
...
Haudenosaunee/Powhatan/Creek/Greek-amphictyony/Latin-League material. `LAW-003` uses
...
finding in the whole audit: an entry (`MIG-007`) names the exact gap, names the exact
...
as a policing/registration mechanism (`LAW-012`, `HLT-010`), never as village
self-governance. `REL-011` covers parish record-keeping, not parish decision-making.
...
Zero hits for "commons" anywhere in the 377-entry corpus. `AGR-005`'s own irrigation
...
`LAW-007` (charters), `COM-015` (Hanseatic League), and `COM-029` (periodic fairs and
...
(`REL-012`), never as an urban-governance actor; futuwwa, muhtasib, and mahalla have
...
`LAW-006` (Estate Tax Treaties — Cortes, Estates, and Parliaments) genuinely covers
...
`AGR-020` (Enclosure) genuinely covers the destruction of one specific corporate
form (open-field customary-use rights). `LAW-015` mentions state-substitutes-for-
...
- **LAW**'s own rule is enforceable obligation. Segmentary opposition, band-to-chief
  ...
  that unit's members owe each other anything enforceable. `LAW-016` and `LAW-008`
  ...
  personhood), and `LAW-008` explicitly names the excluded material as _not_ a legal-
  personhood question.
- **REL**'s own rule is the sacred and its institutions. `REL-018` and `REL-024` both
...
- **DIP**'s own rule is inter-polity relation. A confederation's internal composition —
  ...
  _intra_-group, not inter-polity, which is exactly why `DIP-004` stays at the
...
- **WAR**'s own rule is organized violence and doctrine. The war-band's loyalty
  ...
  is orthogonal to any specific campaign — `MIG-007` already tests this exact boundary
...
- **MIG**'s own rule is movement as a process. Ethnogenesis at a frontier and
  ...
  internal composition isn't itself a movement mechanic, which is why `MIG-007`
...
> actually settles a claim (`LAW-016`) is LAW's.
>
> ## KIN / REL
>
...
>   its priesthood, its calendar, stays REL's (`REL-018`'s existing scope is correct
>   and doesn't change).
>
> A sodality organized around shared devotional practice (a confraternity, a Sufi
> tariqa, `REL-024`) stays REL-canonical. A sodality organized around age, initiation,
...
**LAW-005 note, not reopening the merge:** the already-merged feudalism entry is
...
`LAW-005`'s `domains_touched` as a light-touch fix; no content moves, no merge
reopens.
...
- Mechanism: a lineage reckoned through one line of descent (patrilineal, matrilineal, or cognatic) functions as a bounded corporate body that can hold property, owe and collect collective liability, and outlive any single member — the load-bearing precondition `LAW-016`'s wergild/feud mechanic, `LAW-008`'s explicitly-excluded clan/lineage property case, and `REL-018`'s ancestor cult all already assume.
...
- Prereqs: none (foundational). Enables: `LAW-016`, `LAW-008` (reciprocal fix — both currently assume this structure without citing anything for it), `REL-018` (reciprocal — ancestor cult as this entry's ritual expression).
...
- Answers: Probe 3 directly, and gives `DIP-004`'s elite marriage-diplomacy mechanic its missing ordinary-scale counterpart
...
- Prereqs: `KIN-1` (Descent). See_also: `DIP-004` (explicit elite-scale contrast), `AGR-007` (correcting the record — this is where the corpus's actual Boserup content lives, distinct from this entry's Goody-adjacent territory).
...
- Prereqs: `KIN-3` (as the emergent-leadership branch outcome). See_also: `PHI-036` (explicit contrast — pre-state cyclical complexity vs. already-consolidated dynastic cohesion decline).
...
- Answers: Probe 10's specific missing piece — completes the dependency `MIG-007` itself names and hands to WAR, which does not deliver it
...
- Hook (seed): a branch — confederation forms and holds only while a rich, reachable settled neighbor exists to extract from; remove the neighbor (or its wealth) and the same steppe population reverts to `KIN-3`'s segmentary baseline, matching `MIG-007`'s own already-written "fast-forming, fast-dissolving" cohesion stat with an actual formation mechanism to attach it to.
- Prereqs: `MIG-007` (mobility infrastructure), `KIN-3` (segmentary baseline it supersedes). Enables: nothing further required. See_also: `PHI-036`, `DIP-004` (heqin as the settled empire's own side of the same extraction relationship).
...
- Answers: Probe 4 directly, and supplies `HLT-026`'s currently-missing pre-1800 prereq
- Mechanism: the Hajnal line marks a real, mapped boundary between two structurally different household-formation regimes — northwestern Europe's late female marriage age, high permanent celibacy rate, neolocal residence (a new couple forms an independent household rather than joining an existing one), and life-cycle service (young unmarried adults work in another household before marrying), against the joint/extended family (Chinese zu, Indian joint family, Balkan zadruga, Russian dvor) where multiple married generations or siblings share one household and property. This is a fertility brake operating through _household formation rules_, prior to and independent of `HLT-026`'s later industrial-era mortality/fertility mechanism.
- Hook (seed): a modifier — the Hajnal-line household-formation rule set caps population growth through delayed and sometimes-foregone marriage entirely, a distinct mechanism from `HLT-026`'s later economic-cost-of-children fertility brake, operating centuries earlier and through a different lever (nuptiality, not fertility-within-marriage).
- Prereqs: `KIN-2` (Alliance/marriage). Enables: `HLT-026` (reciprocal fix — HLT-026 currently has no pre-1800 prereq at all despite opening with "mortality falls first... across this entire domain's earlier entries," implying one should exist).
...
the corpus already caught itself on: `MIG-007` names the exact missing mechanism,
